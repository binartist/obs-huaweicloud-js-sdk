...
function encodeURIWithSafe(str, safe, skipEncoding) {
  ...
  if (safe) {
    ...
    for (const element of str) {
      ret.push(
        safe.indexOf(element) >= 0 ? element : encodeURIComponent(element)
      )
    }
    ret = ret.join('')
  } else {
...
function createV4Signature(shortDate, sk, region, stringToSign) {
  let dateKey = crypto
    ...
    .update(shortDate)
    .digest()
  let dateRegionKey = crypto
    .createHmac('sha256', dateKey)
    .update(region)
    .digest()
  let dateRegionServiceKey = crypto
  ...
  if (opt[key] === undefined) {
    if (model[key].type === 'object') {
      opt[key] = model[key].parameters
        ? this.jsonToObject(model[key].parameters, null, null, ifRootXMlDecode)
        : {}
    } else if (model[key].type === 'array') {
      opt[key] = []
    }
            ...
            if (signatureContext.headerMetaPrefix === sentAs) {
              for (let item in _value) {
                if ({}.hasOwnProperty.call(_value, item)) {
                  ...
                  exheaders[item.indexOf(sentAs) === 0 ? item : sentAs + item] =
...
Utils.prototype.getRequest = function (
  ...
  log.runLog(
    'debug',
    methodName,
    'response msg :statusCode:' + serverback.status + ', headers:' + headersStr
  )
  ...
  if (
    ...
    if (location) {
      ...
      redirectErr.bucketLocation =
        headers['x-amz-bucket-region'] || headers['x-obs-bucket-region']
...
Utils.prototype.makeRequest = function (methodName, opt, retryCount, bc) {
  ...
  if (this.ak && this.sk && methodName !== negotiateMethod) {
    if (this.securityToken) {
      opt.headers[signatureContext.headerPrefix + 'security-token'] =
        this.securityToken
    }
    if (isV4) {
    ...
    try {
      ...
      if (err.toString) {
        log.runLog(
          'error',
          methodName,
          'Send request to service error [' + err.toString() + ']'
        )
      }
    }
    log.runLog(
  ...
  if (!this.useRawXhr) {
    ...
    if (
      this.urlPrefix &&
      isFunction(this.setRequestHeaderHook) &&
      ...
      if (
        opt.$requestParam['hasRegion'] ||
        opt.$requestParam['redirectRegion']
      ) {
        defaultRegion = false
      }
...
Utils.prototype.sendRequest = function (funcName, opt, backcall, retryCount) {
  ...
  that.makeRequest(funcName, opt, retryCount, function (err, msg) {
    if (err && err.message === 'redirect') {
      ...
      if (
        err.bucketLocation &&
        uri.hostname.indexOf(opt.$requestParam['Bucket']) !== -1
      ) {
