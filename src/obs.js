...
ObsClient.prototype.restoreFailedWorkflowExecution = function (
  param,
  callback
) {
...
ObsClient.prototype.putObject = function (param, callback) {
  ...
  if (!('ContentType' in param)) {
    if ('Key' in param) {
      param.ContentType =
        this.util.mimeTypes[param.Key.substring(param.Key.lastIndexOf('.') + 1)]
    }

    if (!param.ContentType && 'SourceFile' in param) {
      let fileName = param.SourceFile.name
      param.ContentType =
        this.util.mimeTypes[fileName.substring(fileName.lastIndexOf('.') + 1)]
    }
  }
